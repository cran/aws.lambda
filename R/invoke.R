...
#' @template name
#' @template qualifier
#' @param payload Optionally, the JSON body to send to the function.
#' @param type A character string specifying one of: (1) \dQuote{Event} (asynchronous execution), (2) \dQuote{RequestResponse} (the default), or (3) \dQuote{DryRun} to test the function without executing it.
#' @param log A character string to control log response.
#' @template dots
#' @seealso \code{\link{create_function}}, \code{\link{list_functions}}, 
#' @export
invoke_function <- 
function(name, qualifier, payload = NULL, 
    ...
    h <- list("X-Amz-Invocation-Type" = match.arg(type),
